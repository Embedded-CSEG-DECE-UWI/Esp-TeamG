...
static uint16_t adc_data_red [300], adc_data_ir [300];
...
static void adc_sample(void *arg)
{
    for (;;)
    {
        ...
        TickType_t bstart = xTaskGetTickCount();
        TickType_t now;
        while ((i < 300) && ((now = xTaskGetTickCount()) - bstart) < (10000 / portTICK_PERIOD_MS))
        {
            ...
            ret = adc_channel_sel(I2C_EXAMPLE_MASTER_NUM, 1);
            if (ret == ESP_OK)
            {
            }
            else
